...
class BuildCheckApp(MDApp):
    def __init__(self, **kwargs):
        ...
        self.app_data_dir = Path.home() / ".ABC"
        self.init_data_files()
        self.check_and_update_data_from_github()
    ...
    def load_component(self, filename):
        full_path = self.app_data_dir / filename
        if full_path.exists():
    ...
    def init_data_files(self):
        ...
        data_files = [
            "cpu.json", "gpu.json", "ram.json", "mainboard.json",
            "psu.json", "os.json", "steam_games_cache.json", "version.txt"
        ]
        for filename in data_files:
            dest_file = self.app_data_dir / filename
            if not dest_file.exists():
                if getattr(sys, 'frozen', False):
                    src_path = Path(sys._MEIPASS) / "datas" / filename
                else:
                    src_path = Path(__file__).parent / "datas" / filename
                if src_path.exists():
                    with open(src_path, 'r', encoding='utf-8') as src, open(dest_file, 'w', encoding='utf-8') as dst:
                        dst.write(src.read())

    def check_and_update_data_from_github(self):
        try:
            r = requests.get("https://raw.githubusercontent.com/VanDinh-github/datas/main/version.json")
            self.remote_version = r.json()["version"]
            self.current_version = self.get_local_version()  #  cập nhật bản hiện tại

            if version.parse(self.remote_version) > version.parse(self.current_version):
                print(f"Đang cập nhật từ {self.current_version} → {self.remote_version}")

                file_names = [
                    "cpu.json", "gpu.json", "ram.json", "mainboard.json",
                    "psu.json", "os.json", "steam_games_cache.json"
                ]

                for name in file_names:
                    url = f"https://raw.githubusercontent.com/VanDinh-github/datas/main/{name}"
                    res = requests.get(url)
                    if res.status_code == 200:
                        (self.app_data_dir / name).write_text(res.text, encoding='utf-8')

                self.save_local_version(self.remote_version)
                self.current_version = self.remote_version  #  cập nhật lại sau khi lưu
                self.data_source = "github"
                print("Cập nhật dữ liệu thành công.")
            else:
                print(" Dữ liệu đã là mới nhất.")
                self.data_source = "local"

        except Exception as e:
            print(" Không thể kiểm tra phiên bản GitHub:", e)
            self.current_version = self.get_local_version()  # fallback nếu lỗi
            self.data_source = "local"

    def update_all_files(self):
        base_url = "https://raw.githubusercontent.com/VanDinh-github/datas/main/"
        file_names = [
            "cpu.json", "gpu.json", "ram.json", "mainboard.json",
            "psu.json", "os.json", "steam_games_cache.json"
        ]
    ...
    def save_local_version(self, new_version):
        version_file = self.app_data_dir / "version.txt"
    ...
    def select_component(self, text_field, component, component_type):
        ...
        if component_type == 'cpu':
            ...
            screen.ids.cpu_tdp.text = str(component.tdp)
            ...
            screen.ids.ram_type.text=component.ram_type
            screen.ids.ram_color.text=component.color
            ...
            screen.ids.mainboard_max.text=str(component.max_memory)
            screen.ids.slots.text=str(component.memory_slots)
    ...
    def check_pc_compatibility(self):
        ...
        cpu = CPU(
            ...
            core_count=int(float(screen.ids.cpu_cores.text)) if screen.ids.cpu_cores.text else None,
            socket=screen.ids.cpu_socket.text,
            tdp=int(float(screen.ids.cpu_tdp.text)) if screen.ids.cpu_tdp.text else 0,
            core_clock=float(screen.ids.cpu_coreclock.text) if screen.ids.cpu_coreclock.text else None,
        ...
        gpu = GPU(
            ...
            tdp=int(float(screen.ids.gpu_tdp.text)) if screen.ids.gpu_tdp.text else 0,
            chipset=screen.ids.gpu_chipset.text,
            core_clock=int(float(screen.ids.gpu_coreclock.text)) if screen.ids.gpu_coreclock.text else None,
            length=int(float(screen.ids.gpu_length.text)) if screen.ids.gpu_length.text != 'None' else None,
            color=screen.ids.gpu_color.text
        )
        ram = RAM(
            name=screen.ids.ram_model.text,
            capacity=int(float(screen.ids.ram_capa.text)) if screen.ids.ram_capa.text else None,
        ...
        psu = PSU(
            ...
            wattage=int(float(screen.ids.psu_wattage.text)) if screen.ids.psu_wattage.text!='None' else 500
        )
